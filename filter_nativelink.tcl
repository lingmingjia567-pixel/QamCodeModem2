...
catch {set coef_files [glob -nocomplain filter_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VERILOG"
set ipfs_ext vo
if {[file exists filter.vo]} {
	set hdl_ext v
} else {
	puts "Warning: Could not find filter.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_filter -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE filter.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE filter_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_filter.vhd -section_id tb -library work
