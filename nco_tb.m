...
phi_inc_i = zeros(1,N);
% Construct interleaved multi-channel input phase increment vector
phi_inc_i(1:2:N) = 42949673.*ones(1,N/2);
phi_inc_i(2:2:N) = 21474836.*ones(1,N/2);
...
xvalst = (0:1/fs:((N/2)-1)/fs);
figure,stairs(xvalst,sin_out(1:2:N),'r');
hold on, stairs(xvalst,cos_out(1:2:N),'b');
legend('sine','cosine');
...
ncofftwinplot(sin_out(1:2:N),fs,0,'r');
ncofftwinplot(cos_out(1:2:N),fs,1,'b');
